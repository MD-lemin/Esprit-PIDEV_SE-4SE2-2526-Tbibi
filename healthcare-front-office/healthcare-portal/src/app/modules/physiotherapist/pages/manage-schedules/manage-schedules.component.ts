    ...
    <div class="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 p-6">
      <div class="max-w-5xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">🦴 Mes Disponibilités</h1>
            <p class="text-gray-500 mt-1">Configurez vos créneaux de kinésithérapie</p>
          </div>
          <div class="flex gap-4">
            <div class="bg-white border border-gray-200 rounded-2xl px-5 py-3 text-center shadow-sm">
              <p class="text-2xl font-bold text-green-600">{{slots.length}}</p>
              <p class="text-xs text-gray-400 mt-0.5">Total Créneaux</p>
            </div>
            <div class="bg-white border border-gray-200 rounded-2xl px-5 py-3 text-center shadow-sm">
        ...
        <div class="flex gap-1 bg-gray-100 p-1 rounded-2xl w-fit">
          <button (click)="activeTab='template'"
            [class]="activeTab==='template' ? 'bg-white text-green-700 shadow-sm font-semibold' : 'text-gray-500 hover:text-gray-700'"
            class="px-6 py-2.5 rounded-xl text-sm transition-all">
            ⚙️ Modèle Horaire
          </button>
          <button (click)="activeTab='slots'"
            [class]="activeTab==='slots' ? 'bg-white text-green-700 shadow-sm font-semibold' : 'text-gray-500 hover:text-gray-700'"
            class="px-6 py-2.5 rounded-xl text-sm transition-all">
            🗂 Mes Créneaux
          </button>
        </div>
        ...
        <div *ngIf="activeTab==='template'"
          ...
          <div>
            ...
            <p class="text-sm text-gray-500">Définissez vos horaires et le système génère tous les créneaux jusqu'au <strong>31 décembre {{currentYear}}</strong>.</p>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">🕗 Début travail</label>
            ...
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">🕔 Fin travail</label>
              <input type="time" [(ngModel)]="tpl.workEnd"
                class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-400">
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">⏱ Durée séance (min)</label>
              <input type="number" [(ngModel)]="tpl.consultationMinutes" min="15" max="120" step="5"
                class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-400">
            </div>
          </div>
          ...
          <div>
            ...
            <div *ngIf="tpl.unavailableWindows.length === 0" class="text-sm text-gray-400 italic mb-3">Aucune pause. Cliquez ci-dessous pour en ajouter.</div>
            <div class="space-y-2 mb-3">
              <div *ngFor="let w of tpl.unavailableWindows; let i = index"
                ...
                <button (click)="removeWindow(i)" class="text-red-400 hover:text-red-600 text-lg leading-none">✕</button>
              </div>
            </div>
            <button (click)="addWindow()" class="text-green-600 hover:text-green-800 text-sm font-semibold flex items-center gap-1">
              <span class="text-lg leading-none">+</span> Ajouter pause
            </button>
          </div>

          <div class="flex gap-3 pt-2 border-t border-gray-100">
            <button (click)="generateSlots()" [disabled]="!canGenerate() || generating"
              class="px-6 py-3 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white font-semibold rounded-xl text-sm transition-colors flex items-center gap-2">
        ...
        <div *ngIf="activeTab==='slots'">
          ...
          <div *ngIf="!loading && slots.length === 0"
            ...
            <p class="text-gray-400 text-sm">Allez dans l'onglet <strong>Modèle Horaire</strong> et cliquez "Générer le planning".</p>
          </div>
          <div *ngIf="!loading && slots.length > 0" class="space-y-4">
            <div *ngFor="let group of groupedSlots" class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
              <div class="bg-gradient-to-r from-green-50 to-emerald-50 border-b border-gray-200 px-5 py-3 flex items-center justify-between">
                ...
                <button *ngIf="group.availableCount > 0" (click)="clearAvailableSlotsByDate(group.date)"
                  class="text-xs bg-red-100 text-red-600 px-3 py-1.5 rounded-lg hover:bg-red-200 transition-colors font-semibold">
              ...
              <div class="flex flex-wrap gap-2 p-4">
                <div *ngFor="let slot of group.slots"
                  ...
                  [class]="slot.isAvailable ? 'bg-green-50 border-green-200 text-green-800' : 'bg-orange-50 border-orange-200 text-orange-700'">
                  ...
                  <button *ngIf="slot.isAvailable" (click)="deleteSlot(slot)"
                    class="text-red-400 hover:text-red-600 text-xs ml-1">✕</button>
                </div>
              </div>
            </div>
          </div>
...
export class PhysioManageSchedulesComponent implements OnInit {

  activeTab: 'template' | 'slots' = 'template';
  ...
  loadSlots() {
    this.loading = true;
    this.http.get<ScheduleSlot[]>(`${this.base}/api/doctor/schedules/doctor/${this.physioId}`)
  ...
  clearAvailableSlotsByDate(date: string) {
    ...
    this.http.delete(`${this.base}/api/doctor/schedules/doctor/${this.physioId}/available/date/${date}`).subscribe({
  ...
  generateSlots() {
    ...
    const body = { doctorId: this.physioId, ...this.tpl };
  ...
  clearAndGenerate() {
    ...
    this.generating = true; this.tplError = ''; this.tplSuccess = '';
    this.http.delete(`${this.base}/api/doctor/schedules/doctor/${this.physioId}/available`).subscribe({
      next: () => {
        const body = { doctorId: this.physioId, ...this.tpl };
        this.http.post<any[]>(`${this.base}/api/doctor/schedules/generate`, body).subscribe({
          next: (newSlots) => {
